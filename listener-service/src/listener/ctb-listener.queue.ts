import { QUEUE_CASH_MVT } from '../queues';
import {
  InjectQueue,
  OnGlobalQueueActive,
...
export class CtbListenerQueue {
  constructor(@InjectQueue(QUEUE_CASH_MVT) private cashMouvementQueue: Queue) {}

  @OnGlobalQueueWaiting()
  async onGlobalQueueWaiting(jobId: number | string) {
    const job: Job = await this.cashMouvementQueue.getJob(jobId);
    console.log(
